...
- **Busca manual por palavra + pílula em duas linhas (pedido do usuário).** A pílula (`index.html`, landing) virou `.hero-busca-grupo`: um wrapper flex que centraliza junto `.pill-nav` (agora com **duas linhas** — `.pill-nav__linha--principal` só com o CTA "O que você está sentindo hoje?" em largura total, lido pelo usuário como uma "busca guiada"; `.pill-nav__linha--categorias` com os atalhos de sempre, separada por um fio sutil) e um botão circular satélite **fora** da pílula (`.busca-fab`, 🔍) — decisão de posição confirmada com o usuário via pergunta (opções eram: fim da 2ª linha / início da 2ª linha / fora da pílula — escolheu "fora"). O `.busca-fab` reaproveita a mesma receita de vidro da pílula (blur+saturate+borda+inset highlight).
